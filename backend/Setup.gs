...
/** One-time additive migration. Appends only Withdrawal_Amount to the exact legacy Monthly_Performance contract. */
function addMonthlyPerformanceWithdrawalColumn(){beginRequest_();var s=sheet_('Monthly_Performance'),legacy=SCHEMA.Monthly_Performance.slice(0,-1),actual=s.getRange(1,1,1,Math.max(s.getLastColumn(),legacy.length)).getDisplayValues()[0].map(function(x){return String(x).trim()});while(actual.length&&!actual[actual.length-1])actual.pop();if(actual.length===SCHEMA.Monthly_Performance.length&&actual[actual.length-1]==='Withdrawal_Amount')return {changed:false,column:actual.length};if(actual.length!==legacy.length||!legacy.every(function(h,i){return actual[i]===h}))throw new Error('Migration refused: Monthly_Performance does not match the documented legacy header contract.');s.getRange(1,legacy.length+1).setValue('Withdrawal_Amount');clearCache_('Monthly_Performance');return {changed:true,column:legacy.length+1}}
