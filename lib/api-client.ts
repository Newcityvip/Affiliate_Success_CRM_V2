...
export type AffiliateDetailResponse = { profile:AffiliateDirectoryItem;recentActivity:AffiliateActivity[];performance?:{period:string;current:PerformanceMetrics|null;previous:PerformanceMetrics|null};canUpdatePerformance?:boolean };
...
export type PerformanceItem={affiliateId:string;affiliateUsername:string;affiliateName:string;brandId:string;brandName:string;staffId:string;staffName:string;period:string;status:'UPDATED'|'NOT_UPDATED';performanceId:string;metrics:PerformanceMetrics|null;previous:PerformanceMetrics|null;comparisons:Record<string,{direction:string;percent:number|null}>;needsAttention:boolean;attentionReasons:string[];lastUpdated:string};
export type PerformanceWorkspace={period:string;canManage:boolean;items:PerformanceItem[];summary:{assignedAffiliates:number;updated:number;notUpdated:number;needsAttention:number;registeredUsers:number;ftd:number;activePlayers:number;totalDeposit:number;totalWithdrawal:number;totalTurnover:number;profitLoss:number};staff:Array<{staffId:string;staffName:string;teamId:string;teamName:string;assignedAffiliates:number;updated:number;updatedPercent:number;needsAttention:number;registeredUsers:number;ftd:number;activePlayers:number;totalDeposit:number;totalWithdrawal:number;totalTurnover:number;profitLoss:number;ftdPerAffiliate:number;activePlayersPerAffiliate:number;depositPerAffiliate:number;turnoverPerAffiliate:number}>;options:{staff:Array<{staffId:string;displayName:string;teamId:string}>;teams:Array<{teamId:string;teamName:string}>;brands:Array<{brandId:string;brandName:string}>}};
...
class ApiClient {
  ...
  saveAffiliatePerformance(payload:Record<string,unknown>) { return this.call<{performanceId:string;created:boolean;metrics:PerformanceMetrics}>('saveAffiliatePerformance',payload).then(result=>{invalidateReadCache('performance','affiliate','staff-dashboard','super-admin-dashboard');return result}); }
