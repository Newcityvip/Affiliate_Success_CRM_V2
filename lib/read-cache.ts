...
export function invalidateReadCache(...prefixes:string[]){const targets=[...prefixes,'reports'];for(const key of cache.keys())if(targets.some(prefix=>key===prefix||key.startsWith(`${prefix}:`)))cache.delete(key)}
