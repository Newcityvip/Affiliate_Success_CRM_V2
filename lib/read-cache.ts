...
export function invalidateReadCache(...prefixes:string[]){for(const key of cache.keys())if(prefixes.some(prefix=>key===prefix||key.startsWith(`${prefix}:`)))cache.delete(key)}
