...
Create one V2 spreadsheet and run `setupSpreadsheet()` from `backend/Setup.gs`. The exact headers are also the authoritative `SCHEMA` constant in `backend/Schema.gs`.
...
IDs such as `AFF_…`, `ASN_…`, `WRK_…`, and `INT_…` are permanent; row numbers are never identifiers. Affiliate identity and assignments are separate. Transfers end an assignment and append another. Contact attempts, meaningful interactions, imports, and audits are append-only. Archiving changes lifecycle state and preserves every related record.

Reads fetch a sheet once per operation, build maps in memory, paginate scoped results, and batch writes with `setValues()`. Catalogs use `CacheService`; imports, transfers, and state transitions use `LockService` where duplicates or races matter.
