...
3. Add these repository files to the Apps Script project: `Admin.gs`, `Api.gs`, `Auth.gs`, `Config.gs`, `Dashboard.gs`, `Directory.gs`, `Followups.gs`, `Import.gs`, `Interactions.gs`, `Issues.gs`, `Schema.gs`, `Services.gs`, `Setup.gs`, `StaffDashboard.gs`, `Store.gs`, `Tasks.gs`, and `Workflow.gs`. In Project Settings, enable **Show "appsscript.json" manifest file in editor**, then replace it with `backend/appsscript.json`.
